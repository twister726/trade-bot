...
class FtxWebsocketClient(WebsocketManager):
    ...
    def __init__(self, api_key, api_secret, trade_window, WINDOW_T) -> None:
        super().__init__()
...
#         return list(self._trades[market].copy())
        return list(self.trade_window.copy())
...
#         self._trades[message['market']].append(trade)
        ...
        if self.oldest_trade_time == None:
            self.oldest_trade_time = datetime.datetime.utcnow().timestamp()
            return
        
        while datetime.datetime.utcnow().timestamp() - self.oldest_trade_time >= self.WINDOW_T:
            self.trade_window.popleft()
            self.oldest_trade_time = parse_datetime(self.trade_window[0]['time']).timestamp()
